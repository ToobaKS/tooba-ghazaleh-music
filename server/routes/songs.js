...
router.get("/genre/:id", async (req, res) => {
  ...
  while (!lyrics) {
    ...
    while (fullLyrics == "Lyrics not found") {
      randomSong = pickRandomSong(songs.data);
      fullLyrics = await getLyrics(randomArtist.name, randomSong.title);
    }
    ...
    const detectedLanguage = franc(firstLyricsSnippet);
    console.log(
      `Detected language for ${randomSong.title}: ${detectedLanguage}`
    );
    if (detectedLanguage === "eng") {
      console.log(`Using lyrics for: ${randomSong.title}`);
      lyrics = firstLyricsSnippet;
    } else {
      console.log(`Skipping ${randomSong.title}, not in English`);
    }
  }
  res.json({
